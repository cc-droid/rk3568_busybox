...
#else
static void count_a2dp_packet_timeout(struct timer_list * list);
static void count_pan_packet_timeout(struct timer_list * list);
static void count_hogp_packet_timeout(struct timer_list * list);
#endif
...
static void rtk_check_timer_setup(int8_t profile_index)
{
    if(profile_index == profile_a2dp) {
       ...
       // timer_setup(&(usb_coex_info.a2dp_count_timer), count_a2dp_packet_timeout, 0);
        timer_setup(&(usb_coex_info.a2dp_count_timer), count_a2dp_packet_timeout, 0);
    ...
    if(profile_index == profile_pan) {
        ...
        timer_setup(&(usb_coex_info.pan_count_timer), count_pan_packet_timeout, 0);
    ...
    if((profile_index == profile_hogp) || (profile_index == profile_voice)) {
        if((0 == usb_coex_info.profile_refcount[profile_hogp]) 
            ...
            timer_setup(&(usb_coex_info.hogp_count_timer), count_hogp_packet_timeout, 0);
...
static void polling_bt_info(struct timer_list * list)
{
    uint8_t temp_cmd[1];
...
static void rtk_handle_bt_info_control(uint8_t* p)
{
    ...
    if(info->polling_enable && !usb_coex_info.polling_enable) {
        timer_setup(&(usb_coex_info.polling_timer), polling_bt_info, 0);
